...
//        list_date_experation.remove(id);
        for (int i =0; i<list_date_experation_end.size(); i++){
            ...
            operation_expiration.delete_date_expiration(list_date_experation_end.get(i));
        }
...
//    time dyal pc dyali
    ...
    public static void  main_start() {
        ...
        List<Promotion> list_experation = op.getAll_Expiration_date();
        if (!list_experation.isEmpty()) {
            for (Promotion e : list_experation) {
                if (!All_op_timer.list_date_experation.containsKey(e.getIdPromo())) {
                    All_op_timer.list_date_experation.put(e.getIdPromo(), e.getDateExpiration());
                }
            }
        }
        timer = new Timer();
